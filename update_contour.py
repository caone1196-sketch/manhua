...
# Contour-revealing sheer white micro lace directive
CONTOUR_DIRECTIVE = "barefoot with bare legs, wearing form-fitting gossamer sheer translucent white micro-lace lingerie tightly accentuating body curves, bust contours, waistline, and silhouette highlights through the translucent fabric, micro-string ties, thin lace choker, maximizing glowing skin"

MASTER_PROMPT_TEMPLATE = """A single tarot card built upon the FIRST reference image (the deck's ornamental card template): keep its symmetric ornamental golden frame, the four corner filigree flourishes, the top-center medallion and the bottom band with the small skull ornament EXACTLY as they are — do not redraw, move, recolor or resize any part of the frame. FULL BLEED: the card itself fills the entire image EDGE TO EDGE, corner to corner. Fill the center panel with the new scene, painted in the EXACT master art style of the SECOND reference image (Korean adult manhwa webtoon style): identical crisp lineart, alluring character design, seductive blushing expression, glossy skin highlights, soft cell shading, glowing volumetric rim lighting, barefoot with bare legs, and form-fitting sheer translucent white micro-lace lingerie tightly accentuating natural body contours and curves through the fabric.
...
THEME: sensual classic tarot in the tradition of vintage art-nude tarot decks and Korean adult manhwa aesthetic — gorgeous young woman entirely at ease in her body, alluring seductive poses, completely barefoot with bare feet and legs, dressed in form-fitting gossamer sheer white micro-lace lingerie accentuating body contours and bust curves through the translucent fabric, thin lace choker, maximizing bare glowing skin with specular glossy highlights.

NEGATIVE: shoes, boots, sandals, footwear, thick heavy armor, heavy cloth robes, medieval heavy dresses, full coverage clothes, extra shirts, shorts, wrong or mismatched title font, modern or sans-serif lettering, extra words, misspelled title, dark margin, empty band, background or mat around the card, drop shadow, vignette, card floating inside the frame, extra or miscounted suit objects, duplicated, fused, cropped or half-hidden objects, extra limbs, extra arms, three arms, extra hand, more than two arms, extra fingers, six-fingered hands, extra heads, distorted anatomy, plastic 3D render, text or numbers anywhere except the bottom title band, watermark, signature, artist name, double border, multiple cards, collage.
...
Character & Outfit design: gorgeous voluptuous woman ({age}), barefoot with bare feet and legs (no footwear), hair: {hair}, eyes: {eyes}, build: {build}, skin: {skin} with glossy specular highlights, signature: {signature}. Outfit: form-fitting gossamer sheer white micro-lace lingerie tightly outlining bust curves, waistline, and body contours through translucent lace, thin lace choker, maximizing bare skin."""
...
for c in cards_data['cards']:
    ...
    char_spec_inline = f"She is {c.get('age', 'young')}, barefoot with bare legs (no footwear), with {c.get('hair', 'golden hair')}, {c.get('eyes', 'alluring eyes')}, glowing {c.get('skin', 'porcelain')} skin, {CONTOUR_DIRECTIVE}."
...
print("Updated cards.json with contour-revealing sheer white micro-lace directive!")
