...
# Ultra-micro string bikini minimal directive
MICRO_STRING_DIRECTIVE = "wearing ultra-micro white string bikini with tiny sheer lace patches barely covering intimate areas, micro-string straps, thin lace choker, maximizing bare skin"

MASTER_PROMPT_TEMPLATE = """A single tarot card built upon the FIRST reference image (the deck's ornamental card template): keep its symmetric ornamental golden frame, the four corner filigree flourishes, the top-center medallion and the bottom band with the small skull ornament EXACTLY as they are — do not redraw, move, recolor or resize any part of the frame. FULL BLEED: the card itself fills the entire image EDGE TO EDGE, corner to corner. Fill the center panel with the new scene, painted in the EXACT master art style of the SECOND reference image (Korean adult manhwa webtoon style): identical crisp lineart, alluring character design, seductive blushing expression, glossy skin highlights, soft cell shading, glowing volumetric rim lighting, and ultra-micro white string bikini lingerie barely covering sensitive areas.
...
Character & Outfit design: gorgeous voluptuous woman ({age}), hair: {hair}, eyes: {eyes}, build: {build}, skin: {skin} with glossy specular highlights, signature: {signature}. Outfit: ultra-micro white string bikini, tiny lace patches barely covering bust and intimate areas, micro-string straps, thin lace choker, maximizing bare skin."""
...
for c in cards_data['cards']:
    ...
    char_spec_inline = f"She is {c.get('age', 'young')}, with {c.get('hair', 'golden hair')}, {c.get('eyes', 'alluring eyes')}, glowing {c.get('skin', 'porcelain')} skin, {MICRO_STRING_DIRECTIVE}."
...
print("Updated cards.json with ultra-micro string bikini directive!")
