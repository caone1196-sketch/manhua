# ACE OF CUPS (A) — Frameless Wet-Manhwa v2 · mode modern

- **Slug:** `cups-ace` | **Group:** cups
- **Emblem:** one overflowing chalice
...
```text
A completely frameless vertical tarot card "ACE OF CUPS", edge-to-edge illustration: no decorative borders, no card frame, no banner ribbon, no numbers, no Roman numerals. The only text is the title "ACE OF CUPS" centered at the very bottom in the same antique-gold serif lettering as the style reference card.
...
SCENE & SYMBOLISM: the card's symbolism re-staged in a modern luxury penthouse streamer studio: one overflowing chalice rendered as a glowing holographic emblem above her, PC monitors and RGB neon ambient lighting, the suit objects arranged on her streaming desk, night city skyline bokeh through floor-to-ceiling windows. Tarot emblem integrated naturally: one overflowing chalice. COUNT LOCK — EXACTLY 1 CHALICE (hard constraint; count before you draw).
The scene contains exactly 1 chalice — not 0, not 2.
Placement is locked: exactly one chalice; the five overflowing streams are water, not cups — do not draw extra chalices in the pool.
...
At the bottom, centered: the title "ACE OF CUPS" in antique-gold serif lettering matching reference 1. No numbers, no Roman numerals, no other text, no frame, no border, no banner, no watermark, no signature. Masterpiece manhwa illustration, portrait 7:12.
```
