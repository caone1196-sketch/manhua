# FIVE OF PENTACLES (5) — Frameless Wet-Manhwa v2 · mode modern

- **Slug:** `pentacles-05` | **Group:** pentacles
- **Emblem:** five pentacles in a church window
...
```text
A completely frameless vertical tarot card "FIVE OF PENTACLES", edge-to-edge illustration: no decorative borders, no card frame, no banner ribbon, no numbers, no Roman numerals. The only text is the title "FIVE OF PENTACLES" centered at the very bottom in the same antique-gold serif lettering as the style reference card.
...
FIGURE: 20 years old, build: **A** mảnh, dáng co mình dưới cửa sổ nhà thờ, hair: long windswept dark-brown hair catching falling snow, eyes: icy-grey nhạt, doe, run rẩy mệt mỏi, skin: porcelain with glossy wet highlights, signature: bớt trắng như sương trên cổ, aura: tuyết, đói, cửa sổ sáng. Pose: graceful S-curve contrapposto where the scene allows (hip popped, back slightly arched, one knee softly bent, barefoot); blushing cheeks, cat eyeliner, softly parted lips with a confident gentle smile.

SCENE & SYMBOLISM: the card's symbolism re-staged in a modern luxury penthouse streamer studio: five pentacles in a church window rendered as a glowing holographic emblem above her, PC monitors and RGB neon ambient lighting, the suit objects arranged on her streaming desk, night city skyline bokeh through floor-to-ceiling windows. Tarot emblem integrated naturally: five pentacles in a church window. COUNT LOCK — EXACTLY 5 PENTACLE COINS (hard constraint; count before you draw).
The scene contains exactly 5 pentacle coins — not 4, not 6.
Placement is locked: exactly five glowing coins set in the tall stained-glass window in a quincunx — one at the top center, two on the middle row, two on the bottom row, 1 + 2 + 2 = 5; all five discs separate and countable.
...
At the bottom, centered: the title "FIVE OF PENTACLES" in antique-gold serif lettering matching reference 1. No numbers, no Roman numerals, no other text, no frame, no border, no banner, no watermark, no signature. Masterpiece manhwa illustration, portrait 7:12.
```
