...
# Ultra-fine gossamer sheer micro-string lingerie, barefoot, no shoes/accessories
GOSSAMER_BAREFOOT_DIRECTIVE = "barefoot with completely bare feet and legs, wearing ultra-fine gossamer sheer translucent white micro-string lingerie barely covering intimate points, micro-string ties, thin lace choker, maximizing bare glowing skin"

MASTER_PROMPT_TEMPLATE = """A single tarot card built upon the FIRST reference image (the deck's ornamental card template): keep its symmetric ornamental golden frame, the four corner filigree flourishes, the top-center medallion and the bottom band with the small skull ornament EXACTLY as they are — do not redraw, move, recolor or resize any part of the frame. FULL BLEED: the card itself fills the entire image EDGE TO EDGE, corner to corner. Fill the center panel with the new scene, painted in the EXACT master art style of the SECOND reference image (Korean adult manhwa webtoon style): identical crisp lineart, alluring character design, seductive blushing expression, glossy skin highlights, soft cell shading, glowing volumetric rim lighting, barefoot with completely bare legs and feet, and ultra-fine gossamer sheer white micro-string lingerie barely covering sensitive points.
...
Character & Outfit design: gorgeous voluptuous woman ({age}), barefoot with bare feet and legs (no shoes/footwear), hair: {hair}, eyes: {eyes}, build: {build}, skin: {skin} with glossy specular highlights, signature: {signature}. Outfit: ultra-fine gossamer sheer white micro-string lingerie, tiny gossamer lace patches barely covering intimate points, micro-string ties, thin lace choker, maximizing bare skin."""
...
for c in cards_data['cards']:
    ...
    char_spec_inline = f"She is {c.get('age', 'young')}, barefoot with completely bare feet and legs (no footwear), with {c.get('hair', 'golden hair')}, {c.get('eyes', 'alluring eyes')}, glowing {c.get('skin', 'porcelain')} skin, {GOSSAMER_BAREFOOT_DIRECTIVE}."
...
print("Updated cards.json with gossamer sheer micro-string and barefoot directive!")
